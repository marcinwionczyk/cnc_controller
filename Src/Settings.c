...
void Settings_StoreStartupLine(uint8_t n, char *line) {
    ...
    EEprom24lc16b_WriteByteArray(addr, (uint8_t *) line, STARTUP_LINE_LEN);
}
...
void Settings_StoreBuildInfo(char *line) {
    ...
    EEprom24lc16b_WriteByteArray(EEPROM_ADDR_BUILD_INFO, (uint8_t *) line, STARTUP_LINE_LEN);
}
...
void Settings_WriteCoordData(uint8_t coord_select, float *coord_data) {
    ...
    EEprom24lc16b_WriteByteArray(addr, (uint8_t *) coord_data, sizeof(float) * N_AXIS);
}
...
void WriteGlobalSettings(void) {
    EEprom24lc16b_WriteByte(0, SETTINGS_VERSION);
    EEprom24lc16b_WriteByteArray(EEPROM_ADDR_GLOBAL, (uint8_t *) &settings, sizeof(Settings_t));
}
...
void Settings_Restore(uint8_t restore_flag) {
    ...
    if (restore_flag & SETTINGS_RESTORE_BUILD_INFO) {
        EEprom24lc16b_WriteByte(EEPROM_ADDR_BUILD_INFO, 0);
        EEprom24lc16b_WriteByte(EEPROM_ADDR_BUILD_INFO + 1, 0); // Checksum
    }
}
...
uint8_t Settings_ReadStartupLine(uint8_t n, char *line) {
    uint32_t addr = n * (STARTUP_LINE_LEN + 1) + EEPROM_ADDR_STARTUP_BLOCK;
    ;
    if (EEprom24lc16b_ReadByteArray(addr, (uint8_t *) line, STARTUP_LINE_LEN)) {
...
uint8_t Settings_ReadBuildInfo(char *line) {
    if(EEprom24lc16b_ReadByteArray(EEPROM_ADDR_BUILD_INFO, (uint8_t *) line, STARTUP_LINE_LEN)){
        // Reset line with default value
        line[0] = 0; // Empty line
...
uint8_t Settings_ReadCoordData(uint8_t coord_select, float *coord_data) {
    uint32_t addr = coord_select * (sizeof(float) * N_AXIS + 1) + EEPROM_ADDR_PARAMETERS;
    if(EEprom24lc16b_ReadByteArray(addr, (uint8_t *) coord_data, sizeof(float) * N_AXIS)){
...
uint8_t ReadGlobalSettings() {
    ...
    uint8_t version = EEprom24lc16b_ReadByte(0);
    if (version == SETTINGS_VERSION) {
        // Read settings-record and check checksum
        if(EEprom24lc16b_ReadByteArray(EEPROM_ADDR_GLOBAL, (uint8_t *) &settings, sizeof(Settings_t))){
            return false;
        }
    } else {
        return false;
        ...
        while (set_idx < AXIS_N_SETTINGS) {
            if (parameter < N_AXIS) {
                ...
                switch (set_idx) {
                    case 0:
                        ...
                        if (value * settings.max_rate[parameter] >
                            (MAX_STEP_RATE_HZ * 60.0)) { return (STATUS_MAX_STEP_RATE_EXCEEDED); }
#endif
                ...
                if ((parameter < AXIS_SETTINGS_INCREMENT) ||
                    (set_idx == AXIS_N_SETTINGS)) { return (STATUS_INVALID_STATEMENT); }
