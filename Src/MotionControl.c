...
static float target_prev[N_AXIS] = {0.0f};
...
void MC_Line(float *target, Planner_LineData_t *pl_data)
{
    ...
    Planner_BufferLine(target, pl_data);
}
...
void MC_Arc(float *target, Planner_LineData_t *pl_data, float *position, const float *offset, float radius,
  uint8_t axis_0, uint8_t axis_1, uint8_t axis_linear, uint8_t is_clockwise_arc)
{
	...
	uint16_t segments = floor(fabs(0.5*angular_travel*radius) / sqrtf(settings.arc_tolerance*(2*radius - settings.arc_tolerance)));
	...
	if(segments) {
		...
		if (pl_data->condition & PL_COND_FLAG_INVERSE_TIME) {
			pl_data->feed_rate *= (float)segments;
			BIT_FALSE(pl_data->condition, PL_COND_FLAG_INVERSE_TIME); // Force as feed absolute mode over arc segments.
		}

		float theta_per_segment = angular_travel / ((float)segments);
		float linear_per_segment = (target[axis_linear] - position[axis_linear])/((float)segments);
		...
		*/
		...
		for(i = 1; i < segments; i++) { // Increment (segments-1).
			if(count < N_ARC_CORRECTION) {
				...
				cos_Ti = cosf((float)i*theta_per_segment);
				sin_Ti = sinf((float)i*theta_per_segment);
