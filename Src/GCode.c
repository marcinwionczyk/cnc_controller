...
uint8_t GC_ExecuteLine(char *line) {
    ...
    while (line[char_counter] != 0)  // Loop until no more g-code words in line.
    {
        ...
        mantissa = roundf(100 * (value - (float) int_value)); // Compute mantissa for Gxx.x commands.
    ...
    */
    ...
    if (gc_block.modal.motion == MOTION_MODE_DRILL || gc_block.modal.motion == MOTION_MODE_DRILL_DWELL ||
        gc_block.modal.motion == MOTION_MODE_DRILL_PECK) {
        ...
        if (axis_command == AXIS_COMMAND_MOTION_MODE) {
            ...
            if (gc_state.modal.motion == MOTION_MODE_LINEAR) {
                ...
                MC_Arc(gc_block.values.xyz, pl_data, gc_state.position, gc_block.values.ijk, gc_block.values.r,
                       ...
                       gc_state.modal.motion == MOTION_MODE_DRILL_PECK) {
                float xyz[N_AXIS] = {0.0f};
                ...
                for (uint8_t repeat = 0; repeat < gc_block.values.l; repeat++) {
                    ...
                    xyz[X_AXIS] = gc_block.values.xyz[X_AXIS] + (delta_x * (float)repeat);
                    xyz[Y_AXIS] = gc_block.values.xyz[Y_AXIS] + (delta_y * (float)repeat);
                    pl_data->condition |= PL_COND_FLAG_RAPID_MOTION; // Set rapid motion condition flag.
                    MC_Line(xyz, pl_data);
                    ...
                    if (gc_state.modal.motion != MOTION_MODE_DRILL_PECK) {
                        ...
                        //-- G83 --//
//                        for (float curr_z = clear_z - gc_block.values.q;
//                             curr_z >= gc_block.values.xyz[Z_AXIS] - 0.001; curr_z -= gc_block.values.q) {
...
//                            MC_Line(xyz, pl_data);
//                        }
                        float curr_z = clear_z - gc_block.values.q;
                        while(curr_z >= gc_block.values.xyz[Z_AXIS] - 0.001f){
                            // Check if target depth exceeds final depth
                            if (curr_z < gc_block.values.xyz[Z_AXIS]) {
                                curr_z = gc_block.values.xyz[Z_AXIS];
                            }
