...
void Stepper_Disable(uint8_t ovr_disable)
{
	...
	if(((settings.stepper_idle_lock_time != 0xFF) || sys_rt_exec_alarm || sys.state == STATE_SLEEP) && sys.state != STATE_HOMING) {
		...
		//LL_mDelay(settings.stepper_idle_lock_time);
...
void Stepper_MainISR(void)
{
	...
	if(st.exec_segment == 0) {
		...
		if(segment_buffer_head != segment_buffer_tail) {
			...
			LL_TIM_SetAutoReload(TIM9, st.exec_segment->cycles_per_tick);
			LL_TIM_OC_SetCompareCH1(TIM9, st.exec_segment->cycles_per_tick * 0.75);
...
void Stepper_PrepareBuffer(void)
{
	...
	while(segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
		...
		*/
		...
		uint32_t cycles = ceilf((float)(TICKS_PER_MICROSECOND*1000000*60)*inv_rate); // (cycles/step)
