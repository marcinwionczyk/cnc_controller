...
uint8_t Planner_BufferLine(const float *target, Planner_LineData_t *pl_data)
{
	...
	for(idx = 0; idx < N_AXIS; idx++) {
		...
		block->steps[idx] = labs(target_steps[idx]-position_steps[idx]);
		block->step_event_count = max(block->step_event_count, block->steps[idx]);
		delta_mm = (float)(target_steps[idx] - position_steps[idx])/settings.steps_per_mm[idx];
#endif
...
float Planner_ComputeProfileNominalSpeed(Planner_Block_t *block)
{
	...
	if(block->condition & PL_COND_FLAG_RAPID_MOTION) {
		nominal_speed *= (0.01f*(float)sys.r_override);
	}
	else {
		if(!(block->condition & PL_COND_FLAG_NO_FEED_OVERRIDE)) {
			nominal_speed *= (0.01f*(float)sys.f_override);
		}
