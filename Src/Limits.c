...
uint8_t Limits_GetState(void)
{
    ...
    limit_state = (LL_GPIO_IsInputPinSet(XLIMIT_GPIO_Port, XLIMIT_Pin)) << X_LIMIT_BIT;
    limit_state |= (LL_GPIO_IsInputPinSet(YLIMIT_GPIO_Port, YLIMIT_Pin)) << Y_LIMIT_BIT;
    limit_state |= (LL_GPIO_IsInputPinSet(ZLIMIT_GPIO_Port, ZLIMIT_Pin)) << Z_LIMIT_BIT;
...
void Limits_GoHome(uint8_t cycle_mask)
{
	...
	do {
		...
		//LL_mDelay(settings.homing_debounce_delay);
