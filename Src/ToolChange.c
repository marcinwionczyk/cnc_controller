...
void TC_ProbeTLS(void) {
    ...
    position[Z_AXIS] = (settings.tls_position[Z_AXIS] / settings.steps_per_mm[Z_AXIS]) + 90.0f;
    MC_Line(position, &pl_data);
    ...
    System_ConvertArraySteps2Mpos(position, sys_position);
    position[Z_AXIS] += 1.5f;

    // Move up a little bit for slow probing
    pl_data.feed_rate = 200.0f;
    MC_Line(position, &pl_data);

    // Probe TLS slow
    pl_data.feed_rate = 12.0f;
    position[Z_AXIS] -= 200;
